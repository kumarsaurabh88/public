...
<main id="main" class="main">
  ...
  <table class="table table-borderless datatable">
    ...
    <tbody>
      @if (!$blog_tags->isEmpty())
      @foreach ($blog_tags as $blog_tags)
      <tr>
        <td>{{ $blog_tags->tagname }}</td>
        <td>
          <a href="{{route('adjust.tag', $blog_tags->id)}}" class="btn btn-dark">Edit</a>
          <form id="delete-blog-form-{{$blog_tags->id}}" action="{{route('discard',$blog_tags->id)}}" method="post">
            @csrf
            @method('DELETE')
