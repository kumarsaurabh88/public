...
<main id="main" class="main">
  ...
  <div class="pagetitle">
    ...
    <nav>
      ...
      <a href="{{ route('author') }}" class="btn btn-primary">
        <i class="fas fa-plus-circle">add Author</i>
      </a>
  ...
  <table class="table table-borderless datatable">
    ...
    <tbody>
      @if (!$blog_auths->isEmpty())
      @foreach ($blog_auths as $blog_auths)
      <tr>

        <td>{{ $blog_auths->name }}</td>
        <td>{{ $blog_auths->description }}</td>

        <td>
          @if ($blog_auths->image != "")
          <img src="{{ asset($blog_auths->image) }}" alt="Current Image" style="max-width: 50px; border-radius: 50%;">
          @else
          No image available
          @endif
        </td>
        ...
        <td>
          <a href="{{route('amend.author', $blog_auths->id)}}" class="btn btn-dark">Edit</a>
          <form id="delete-blog-form-{{$blog_auths->id}}" action="{{route('remove',$blog_auths->id)}}" method="post">
            @csrf
            @method('DELETE')
